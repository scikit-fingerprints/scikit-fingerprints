...
class ConformerGenerator(BasePreprocessor):
    ...
    max_gen_attempts : int, default=1000
    ...
    suppress_warnings : bool, default=False
        Whether to suppress warnings on generating conformers, e.g. UFFTYPER warnings.
    ...
    _parameter_constraints: dict = {
        ...
        "random_state": [Interval(Integral, left=-1, right=None, closed="left"), None],
    }
    ...
    def transform_x_y(
        ...
        If ``errors`` is set to ``"filter"``, then in case of errors fewer than
        n_samples values may be returned (``n_samples_conf_gen <= n_samples``).
        If ``errors`` is set to ``"ignore"``, some molecules may be returned without
        conformers generated and with ``conf_id`` property set to -1.
    ...
    def _embed_molecule(self, mol: Mol) -> tuple[Mol, int]:
        ...
        self._set_common_embed_params(embed_params)
        ...
        conf_id = self._run_embedder(mol, embedder, embed_params)
        ...
        if mol.GetNumConformers() == 0 or conf_id == -1:
            # fall back to plain DG
            embed_params = EmbedParameters()
            self._set_common_embed_params(embed_params)
            embed_params.useExpTorsionAnglePrefs = False
            embed_params.useBasicKnowledge = False
            conf_id = self._run_embedder(mol, embedder, embed_params)
    ...
    def _run_embedder(
        self, mol: Mol, embedder: Callable, embed_params: EmbedParameters
    ) -> int:
