...
@pytest.fixture
def smiles_passing_rule_of_gsk() -> list[str]:
    return [
...
@pytest.fixture
def smiles_failing_rule_of_gsk() -> list[str]:
    return [
...
def test_mols_passing_rule_of_gsk(smiles_passing_rule_of_gsk):
    mol_filter = RuleOfGsk()
    smiles_filtered = mol_filter.transform(smiles_passing_rule_of_gsk)
    assert all(isinstance(x, str) for x in smiles_filtered)
    assert len(smiles_filtered) == len(smiles_passing_rule_of_gsk)


def test_mols_partially_passing_rule_of_gsk(smiles_passing_one_fail):
    mol_filter = RuleOfGsk(allow_one_violation=True)
    smiles_filtered = mol_filter.transform(smiles_passing_one_fail)
    assert all(isinstance(x, str) for x in smiles_filtered)
    assert len(smiles_filtered) == len(smiles_passing_one_fail)


def test_mols_failling_rule_of_gsk(smiles_failing_rule_of_gsk):
    mol_filter = RuleOfGsk()
    smiles_filtered = mol_filter.transform(smiles_failing_rule_of_gsk)
    assert all(isinstance(x, str) for x in smiles_filtered)
    assert len(smiles_filtered) == 0


def test_rule_of_gsk_return_indicators(
    smiles_passing_rule_of_gsk,
    smiles_failing_rule_of_gsk,
    ...
    mol_filter = RuleOfGsk(return_indicators=True)
    filter_indicators = mol_filter.transform(all_smiles)
    expected_indicators = np.array(
        [True] * len(smiles_passing_rule_of_gsk)
        + [False] * len(smiles_failing_rule_of_gsk)
        + [False] * len(smiles_passing_one_fail),
        dtype=bool,
    )
    assert np.array_equal(filter_indicators, expected_indicators)

    mol_filter = RuleOfGsk(allow_one_violation=True, return_indicators=True)
    filter_indicators = mol_filter.transform(all_smiles)
    expected_indicators = np.array(
        [True] * len(smiles_passing_rule_of_gsk)
        + [False] * len(smiles_failing_rule_of_gsk)
...
def test_rule_of_gsk_parallel(smiles_list):
    mol_filter = RuleOfGsk()
    mols_filtered_sequential = mol_filter.transform(smiles_list)

    mol_filter = RuleOfGsk(n_jobs=-1, batch_size=1)
