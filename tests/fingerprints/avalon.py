...
def test_avalon_bit_fingerprint(smiles_list, mols_list):
    ...
    X_rdkit = np.array([GetAvalonFP(mol) for mol in mols_list], dtype=np.uint8)
...
def test_avalon_count_fingerprint(smiles_list, mols_list):
    ...
    X_skfp = avalon_fp.transform(smiles_list)
    X_rdkit = np.array(
        [GetAvalonCountFP(mol).ToList() for mol in mols_list], dtype=np.uint32
    )
...
def test_avalon_sparse_bit_fingerprint(smiles_list, mols_list):
    ...
    X_skfp = avalon_fp.transform(smiles_list)
    X_rdkit = csr_array([GetAvalonFP(mol) for mol in mols_list], dtype=np.uint8)
