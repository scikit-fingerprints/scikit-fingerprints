...
def run_test_similarity_and_distance(
    ...
    sim_dense = sim_func(vec_a, vec_b)
    dist_dense = dist_func(vec_a, vec_b)

    sim_sparse = sim_func(vec_a_sparse, vec_b_sparse)
...
def run_test_bulk_similarity_and_distance(
    ...
    expected_sim = [
        [sim_func(fps[i], fps[j]) for j in range(len(fps))] for i in range(len(fps))
    ]
    expected_dist = [
        [dist_func(fps[i], fps[j]) for j in range(len(fps))] for i in range(len(fps))
    ]

    bulk_sim = bulk_sim_func(fps)
    bulk_dist = bulk_dist_func(fps)

    bulk_sim_sparse = bulk_sim_func(fps_sparse)
...
def run_test_bulk_similarity_and_distance_two_arrays(
    ...
    expected_sim = [
        [sim_func(fps_1[i], fps_2[j]) for j in range(len(fps_2))]
    ...
    bulk_sim = bulk_sim_func(fps_1, fps_2)
    bulk_dist = bulk_dist_func(fps_1, fps_2)

    bulk_sim_sparse = bulk_sim_func(fps_sparse_1, fps_sparse_2)
